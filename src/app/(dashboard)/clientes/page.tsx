  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex gap-2">
        ...
        />
        <button className="rounded border border-neutral-300 px-3 py-2 text-sm">Buscar</button>
      </form>

      {perfis.length === 0 ? (
        <p className="text-sm text-neutral-500">
          {busca ? "Nenhum cliente com esse nome." : "Nenhum cliente no espelho ainda — rode a primeira carga em Operação."}
        </p>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-neutral-100">
              {perfis.map((p) => {
                ...
                return (
                  <tr key={p.customerConexaId} className={inelegivel ? "bg-neutral-50 text-neutral-400" : ""}>
                    <td className="px-4 py-2">
                      <Link href={`/clientes/${p.customerConexaId}`} className="hover:underline">
                        {p.customer?.name ?? `Cliente ${p.customerConexaId}`}
                      </Link>
                      {inelegivel ? (
                        <span className="ml-2 rounded bg-neutral-200 px-1.5 py-0.5 text-[10px] uppercase text-neutral-600">
