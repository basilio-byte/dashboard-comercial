...
|---|---|---|---|---|
| ~~R1~~ | ~~`/room/bookings` bloqueado por permissão (403)~~ | ✅ **RESOLVIDO na Fase 0** | Medido em produção: responde **200**, com `deductedFromQuota` em dado real. As 6 regras que dependiam dele seguem viáveis | — |
...
| ~~A8~~ | ~~`sale.quantity` não carrega horas fracionárias~~ | ✅ **REFUTADO na Fase 0.** O achado estava certo em exigir medição e errado na conclusão: **20/20 pares (reserva, venda) concordam**. A coleção Postman é que está errada ao tipar o campo como `integer`/"quantidade de itens" | Rebaixamento revertido. O plano B da regra 4 é viável |
| ~~A9~~ | ~~`/contracts` não devolve `extraFields`~~ | ✅ **REFUTADO na Fase 0** — devolve, 20/20. (Sobra que **nenhum contrato tem o campo preenchido**, o que torna a via uma melhoria futura, não uma solução pronta.) | Irrelevante na prática: a regra 10 se desbloqueou por outro caminho — a **cota do plano**, não o nome nem o extraField |
